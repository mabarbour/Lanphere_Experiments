...
wind_leaf_miners <- read.csv("~/Documents/Lanphere_Experiments/wind_experiment/data/wind_dissections_leaf_miner_summer_2012.csv", stringsAsFactors=F, skip = 1) %>% # this data set focuses on the leaf mining moth, Caloptilia pruniella
  ...
  select(plant_code, 
         ...
         ptoid_LTF_Braconid = ptoid_sp_329) %>%
  group_by(plant_code) %>%
...
aa.2012.vis <- read.csv("~/Documents/Lanphere_Experiments/ant_aphid_experiment/data/Ant-Aphid_Data_v2.csv", stringsAsFactors=F, skip = 1) %>%
  ...
  mutate(X = as.factor(1:1800), # unique ID for each data point
         ...
         leafhopper = Leaf.Hoppers.Brown.Blue + LH.nymph.blk.ylw + Leaf.Hoppers.Camo + leafhopper.brownish + leafhopper.grey + leafhopper.garden.sp7 + scuttler,
...
aphid.growth.df <- aa.2012.vis %>%
  select(Date_rel:plant_code, aphid_Aphis) %>%
  ...
  select(plant_code, week1.growth:week6.growth) %>%
  ...
  select(Date_rel, plant_code, 
         Aphis.growth.rate = Aphid.growth.rate) %>%
  left_join(., select(aa.2012.vis, Date_rel:Dead)) %>%
  select(Date_rel, Block:Aphid.treatment, plant_code, X, Aphis.growth.rate)
...
aa.arth.df <- left_join_NA_to_0(aa.2012.vis.max, aa.leafminers.2012)
glimpse(aa.arth.df)
...
write.csv(aa.arth.df, '~/Documents/Lanphere_Experiments/final_data/ant_aphid_arthropod_df.csv')
