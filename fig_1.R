...
source('~/Projects/miscellaneous_R/model_diagnostic_functions.R')
...
aa.arth.df <- read.csv('~/Projects/Lanphere_Experiments/final_data/ant_aphid_arthropod_df.csv') %>%
  tbl_df() %>%
  mutate(
    X = as.factor(X),
...
wind.arth.df <- read.csv('~/Projects/Lanphere_Experiments/final_data/wind_arthropod_df.csv') %>%
  tbl_df() %>%
...
source('~/Projects/miscellaneous_R/veganCovEllipse.R')
...
b.ord.wind <- ggplot(data = df_ell.cap.b, aes(x = RDA1, y = RDA2, group = Wind.Exposure)) +
  ...
  #ylab("RDA 2 (10%)") + xlab("RDA 1 (14%)") #
  ggtitle("Wind | Rhizosphere bacteria")
b.ord.wind

## ant-aphid: arthropod abundance analysis ----

aa.arth.abund.fact <- glmer(total.abund ~ Ant.mound.dist*Aphid.treatment + Genotype + (1|plant_ID) + (1|Block/fact.Ant.mound.dist), data = aa.arth.df, contrasts = list(Aphid.treatment = "contr.sum", Genotype = "contr.sum"), family = "poisson", control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5))) # model ant mound distance as an ordered factor to make plotting easier. Same qualitative result as when I model it as a continuous predictor. Also, I removed interaction with willow genotype since it was not significant and it allowed the model to converge.

aa.abund.up <- update(aa.arth.abund.fact, .~. -Genotype -Ant.mound.dist*Aphid.treatment + (Aphid.treatment|fact.Ant.mound.dist) + (1|Genotype))
...
a.g.ab <- as.data.frame(Effect("Genotype", aa.arth.abund.fact)) %>% rename(Treatment = Genotype) %>% mutate(Treatment = factor(Treatment, levels = Treatment[order(fit)]), Dummy = factor(Treatment, levels = Treatment[order(fit)])) %>% select(Treatment, Dummy, fit:upper)
...
#a.ab$Dummy <- factor(a.ab$Dummy, labels = c("1 m to Ants", "6 m to Ants", "12 m to Ants", "", as.character(a.g.ab$Treatment)))
a.ab$Treatment <- factor(a.ab$Treatment, labels = c("Aphids", "Control", "", rep(" ",10)))
#a.ab$Dummy <- as.character(a.ab$Dummy)
#a.ab$Dummy[1:6] <- c("1 m to Mound", "1 m to Mound", "6 m to Mound", "6 m to Mound", "12 m to Mound", "12 m to Mound")
#a.ab$Dummy <- as.ordered(a.ab$Dummy)
...
#a.ab.p <- ggplot(a.ab, aes(x = Treatment, y = fit)) + 
 # geom_point(size = p.size) + 
  ...
  #geom_errorbar(aes(ymin = lower, ymax = upper), width = ebar.w*2) + scale_y_continuous(name = "Ant-Aphid Arthropod\nTotal Abundance", breaks = c(0,2,4,6,8,10), limits = c(0,10.5)) + xlab("") + geom_vline(xintercept = 3, linetype = "dashed") + 
...
a.ab.p <- ggplot(a.ab, aes(x = Dummy, y = fit, shape = Treatment, fill = Treatment, color = Treatment)) + 
  ...
  #geom_linerange(x = 4, ymin = 0, ymax = 8, linetype = "dashed", size = 0.5) +
  geom_vline(xintercept = 4, linetype = "dashed") + 
  geom_point(size = p.size) +
  scale_shape_manual(values = c(21,21,32,32)) +
  scale_color_manual(values = c("black","black","white","black"), guide = FALSE) +
  scale_fill_manual(values = c("black","white","white","grey")) +
  theme_cowplot() +
  theme(axis.title = element_text(size = 12),
    legend.justification=c(0,1), legend.position=c(0,1), legend.direction = "vertical", 
    legend.text = element_text(size = 11),
    legend.title = element_blank(), #legend.position = "none",#"top", legend.title = element_blank(),
    axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5, size = 11))
#geom_text(aes(label = Dummy), position = position_dodge(width = pd)) + 
#ggtitle("Total Abundance"); 
a.ab.p #+ geom_rect(xmin = 0.5, xmax = 3.5, ymin = 8, ymax = 9.5, show.legend = FALSE, fill = "white", color = "white")

a.ab.p <- a.ab.p + geom_point(data = a.ab[-(1:7),], aes(x = Dummy, y = fit), size = p.size, fill = "grey", shape = 21)
...
#fig2 <- add_sub(lanphere.fig2, "RDA Axis 1", y = 0, vjust = -1.5, fontface = 'bold'); ggdraw(fig2)
