...
\title{Partitioning plant genetic and environmental drivers of above and belowground communities}
...
%\bigskip
\textit{Running Headline}: G$\times$E drivers of community assembly on plants
...
until there were at least 5 individuals. If plants in the control treatment had any aphids, we also noted their abundance, but removed these aphids from the plant. The ant-aphid experiment was
...
the $(fresh\ mass - dry\ mass)/(dry\ mass)$ (\citealt{Munns_2010}). To
...
ant mound, their 2-way and 3-way interactions as fixed effects in the model.
...
We specified willow genotype, wind treatment, sampling year, their 2-way and 3-way interactions as fixed effects in the model. We lacked multiple years of data on leaf trichome density (2012 only), SLA (2013 only), leaf C:N (2013 only), and root C:N (2013 only) for the wind experiment; therefore, we removed sampling year, and its interactions, from the fixed effects structures of these GLMMs. Plant mortality in each
...
https://github.com/mabarbour/Lanphere\textunderscore Experiments.git. The release associated with this version of the manuscript can be found at https://doi.org/10.5281/zenodo.840033.
...
biotic environment on arthropod communities would be due to, in part,
variation in the abundance of \textit{A. farinosa} and \textit{F.
obscuripes}. While distance from ant mounds had no effect on \textit{A.
...
communities would be due to plant traits. We observed both direct
...
due to plant traits. Interestingly, we found that plant-growth and
...
did vary among willow genotypes (Fig.~\ref{Fig:GxEcomp}C; \(R_{adj}^2=1\%,\ F_{9,117}=1.00,\ P=0.005\)) with no
...
composition of the bacteria community (Fig.~\ref{Fig:GxEcomp}D; \(R_{adj}^2=3\%,\ F_{1,9}=1.38,\ P=0.092\)),
...
on arthropod richness and abundance was due primarily to plant
...
%Similarly, 
While we expected the effects of our aphid treatment to be caused by aphid abundance, the missing paths between aphid treatment and arthropod abundance (Fig.~\ref{aaSEM}B) as well as rarefied richness (Fig.~\ref{aaSEM}C) suggests that the presence of aphids alone influenced arthropod community responses. 
...
(Table~\ref{tab:SEM}). Both trait PC1 and PC2 appeared to cause the indirect effects of
...
relatively stronger (Table~\ref{tab:SEM}). Trait PC1 had a strong, positive effect
...
exposure on community composition were primarily due to plant trait
...
All data and R-scripts for reproducing the reported results are publicly available and can be found at the GitHub repository https://github.com/mabarbour/Lanphere\textunderscore Experiments.git. The release associated with this version of the manuscript can be found at https://doi.org/10.5281/zenodo.840033. Data for these analyses will also be deposited on figshare.
...
obscuripes} (black dots = aphid treatment; white dots = control) (A). In the wind experiment, wind exposure reduced arthropod
...
we only plotted paths with standardized coefficients $>$
0.10, but marginally significant effects (0.05 $<$ P $<$ 0.10) are partially transparent (alpha = 0.5). Numbers next to all arrows represent the standardized path
